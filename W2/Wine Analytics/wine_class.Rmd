...
```{r}
model1$coefficients
ggplot(winetrain, aes(x = VINT, y = LPRICE)) + geom_point(na.rm = TRUE) + geom_smooth(method = "lm", na.rm = TRUE, se = FALSE)
```
...
ggplot(winetrain, aes(x = DEGREES, y = HRAIN, color = cut(LPRICE, c(-Inf, -1.42, Inf)))) + geom_point(na.rm = TRUE) + scale_color_discrete(name = "LPRICE", labels = c("<= mean(LPrice)", "> mean(LPRICE)")) + geom_vline(xintercept = mean(winetrain$DEGREES), color = "blue", lwd = 1) + geom_hline(yintercept = mean(winetrain$HRAIN), color = "red", lwd = 1)

plot(x = winetrain$DEGREES, y = winetrain$HRAIN, col = ifelse(winetrain$LPRICE >= mean(winetrain$LPRICE), "red", "black"))
```
...
# the same values are obtained as from above, beause the model automatically drops the perfectly collinear variable
