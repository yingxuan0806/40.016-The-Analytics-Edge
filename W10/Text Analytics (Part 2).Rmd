...
```{r, message=FALSE}
corpus <- tm_map(corpus, content_transformer(tolower)) # because R is case-sensitive
corpus <- tm_map(corpus, removeWords, stopwords("english"))
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, stemDocument) # reduce all variations of words to root words
```
...
# returns what are the terms used as determinant in the tree nodes
```
